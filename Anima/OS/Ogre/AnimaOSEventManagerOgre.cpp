...
namespace AE
{
	namespace OS
	{
		EventManagerOgre::EventManagerOgre() 
		{
		...
		bool EventManagerOgre::getNextEvent(AE::OS::Event *eventContainer)
		{
		...
		bool EventManagerOgre::peekNextEvent()
		{
		...
		void EventManagerOgre::registerWindowListener(AE::OS::WindowListener *windowListener)
		{
			mWindowListeners.push_back(windowListener);
		}

		void EventManagerOgre::waitNextEvent()
		{
			//al_wait_for_event(mAllegroEventQueue, &mAllegroEvent);
		}

		/*void EventManagerAllegro::_dispatchEvent(ALLEGRO_EVENT *allegroEvent)
		{
			if(allegroEvent->type > ALLEGRO_EVENT_DISPLAY_EXPOSE)
			{
				AE::OS::Window *window = static_cast<AE::OS::WindowManagerAllegro *>(AE::OS::WindowManager::getInstance())->_getWindow(allegroEvent->display.source);
		...
		/*AE::OS::Event* EventManagerAllegro::_translateEvent(ALLEGRO_EVENT *event)
		{
			...
			case ALLEGRO_EVENT_DISPLAY_CLOSE:
				{
					AE::OS::Window *window = static_cast<AE::OS::WindowManagerAllegro *>(AE::OS::WindowManager::getInstance())->_getWindow(event->display.source);
