...
namespace AE
{
	namespace Graphics
	{
		SurfaceDX9::SurfaceDX9(IDirect3DSurface9 *surfaceDX9)
			: mSurfaceDX9(surfaceDX9)
		{
			mSurfaceDX9 = surfaceDX9;
			mSurfaceDX9->GetDesc(&mSurfaceDesc);
			
			// NB: DirectX uses the format ARGB
			// NB 2: A -1 shift value means it doesn't exist
			switch(mSurfaceDesc.Format)
			{
		...
		void SurfaceDX9::blit(AE::Graphics::Surface *sourceSurface, const AE::Math::Point2 &dimensions, const AE::Math::Point2 &position)
		{
			(*mBlitFromSurfaceFunctor)(sourceSurface, dimensions, position);
		}

		void SurfaceDX9::blit(void *source, const AE::Math::Point2 &dimensions, const AE::Math::Point2 &position)
		{
			(*mBlitFunctor)(source, dimensions, position);
		}

		void SurfaceDX9::_blit(AE::Graphics::Surface *sourceSurface, const AE::Math::Point2 &dimensions, const AE::Math::Point2 &position)
		{
		...
		void SurfaceDX9::_blitAlpha(void *source, const AE::Math::Point2 &dimensions, const AE::Math::Point2 &position)
		{
			if(mPixelFormat.colorDepth == 16)
			{
				AE::uint16 strideInBytes = mLockedRect.Pitch / 2;
		...
		void SurfaceDX9::_blitNoAlpha(void *source, const AE::Math::Point2 &dimensions, const AE::Math::Point2 &position)
		{
			if(mPixelFormat.colorDepth == 16)
			{
				AE::uint16 strideInBytes = mLockedRect.Pitch / 2;
		...
		void SurfaceDX9::clear(const AE::Graphics::Color &backgroundColor)
		{
		...
		void* SurfaceDX9::getData()
		{
			return mLockedRect.pBits;
		}

		AE::Math::Point2 SurfaceDX9::getDimensions()
		{
			return AE::Math::Point2(mSurfaceDesc.Width, mSurfaceDesc.Height);
		}

		void SurfaceDX9::lock(AE::Graphics::LockType lockType)
		{
			DWORD d3dLock;
			switch(lockType)
			{
		...
		void SurfaceDX9::plot(const AE::Graphics::Color &color, const AE::Math::Point2 &position) 
		{
			if(mPixelFormat.colorDepth == 16)
			{
		...
		void SurfaceDX9::plot32(const AE::uint32 color, const AE::Math::Point2 &position) 
		{
